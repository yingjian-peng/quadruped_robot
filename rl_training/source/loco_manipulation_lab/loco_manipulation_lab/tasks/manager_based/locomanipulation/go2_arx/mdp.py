"""Task-specific observation and reward terms for Go2-ARX."""
...
def end_effector_position_b(env, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    """Return the selected end-effector position in the robot root frame."""
...
def pose_command_position(env, command_name: str) -> torch.Tensor:
    """Return the Cartesian part of an Isaac Lab pose command."""
...
def pose_command_position_error(env, command_name: str, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    """Squared root-frame Cartesian error between the gripper and its target."""
...
def pose_command_position_delta(env, command_name: str, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    """Return the root-frame Cartesian displacement from target to gripper."""
    return end_effector_position_b(env, asset_cfg) - pose_command_position(env, command_name)


def pose_command_position_exp(
    env, command_name: str, asset_cfg: SceneEntityCfg, std: float
) -> torch.Tensor:
    """Exponentiated end-effector position tracking reward."""
...
def relative_height_scan(env, sensor_cfg: SceneEntityCfg, offset: float, scale: float) -> torch.Tensor:
    """Match the legacy height observation: ``clip(base_z - offset - hit_z)``."""
