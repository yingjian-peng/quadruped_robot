"""Isaac Lab manager-based Go2-ARX locomotion-manipulation environment."""
...
from loco_manipulation_lab.assets import GO2_ARX_CFG

from . import mdp as go2_arx_mdp
...
CONTROLLED_JOINTS = LEG_JOINTS + ARM_JOINTS
GRIPPER_CFG = SceneEntityCfg("robot", body_names="arm_link6")


@configclass
class Go2ArxSceneCfg(InteractiveSceneCfg):
    """Plane, robot, contact sensor, and legacy-compatible height scanner."""
    ...
    robot = GO2_ARX_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        gripper_pos = ObsTerm(func=go2_arx_mdp.end_effector_position_b, params={"asset_cfg": GRIPPER_CFG})
        gripper_goal = ObsTerm(
            func=go2_arx_mdp.pose_command_position,
            params={"command_name": "gripper_pose"},
        )
        gripper_error = ObsTerm(
            func=go2_arx_mdp.pose_command_position_delta,
    ...
    class CriticCfg(PolicyCfg):
        height_scan = ObsTerm(
            func=go2_arx_mdp.relative_height_scan,
...
class RewardsCfg:
    ...
    base_height = RewTerm(func=mdp.base_height_l2, weight=-0.2, params={"target_height": 0.25})
...
@configclass
class Go2ArxFlatEnvCfg(ManagerBasedRLEnvCfg):
    seed = 42
    scene: Go2ArxSceneCfg = Go2ArxSceneCfg(num_envs=4096, env_spacing=3.0)
